...
0 1 2 3 4 5(high)
*/
ll pow_19[9] = {1, 19, 361, 6859, 130321, 2476099, 47045881, 893871739, 16983563041};
ll _map[120];
...
ll password2int(const char *str)
{
    ll r = 0;
    for (int i = 0; i < strlen(str); i++)
    {
        r += _map[str[i]] * pow_19[i];
    }
    return r;
}

bool int2password(char *str, ll pw)
{
    ll i = 0;
    while (pw > 0)
    {
...
public:
    ...
    HashItem(uint k, ll v) : key(k), value(v) {}
};
class HashTable
{
...
public:
    ...
    ll get(uint k)
    { //-1 : No, -2 : Duplicated, Other : password(int)
        int h = hash(k);
        HashItem *hp = table[h];
        if (hp == nullptr)
            return -1;
...
void pre_put()
{
    //"0" -> "aaaaa"
    char str[6];
    ll m = password2int("aaaaa");
    for (ll i = 1; i <= m; i++)
    {
        if (int2password(str, i) == false)
            continue;
...
{
    //find
    ll result = hashtable.get(password);
    char str[10];
    if (result == -1)
    {
        printf("No\n");
